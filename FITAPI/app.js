  ...
  try {
    ...
    const response = await database.createDocument(
      ...
      ID.unique(),
      {
        ...
        Date: new Date(fitnessData.date).toISOString(), // Adjust the date format if necessary
        stepCount: fitnessData.step_count.toString(),
        Height: fitnessData.height_in_cms.toString(), // Ensure it's a string
        Weight: fitnessData.weight.toString(), // Ensure it's a string or a valid numeric value
        menstrualCycle: fitnessData.menstrual_cycle_start.toString(),
        HeartRate: fitnessData.heart_rate.toString(), // Ensure it's a string or a valid numeric value
        glucoseLevel: fitnessData.glucose_level.toString(), // Ensure it's a string or a valid numeric value
        bodyFat: fitnessData.body_fat_in_percent.toString(), // Ensure it's a string or a valid numeric value
        bloodPressure: fitnessData.blood_pressure.toString(), // Ensure it's a string or a valid numeric value
...
const viewdata = async () => {
  try {
    var x = await appwrite.database.listDocuments(
      process.env.DATABASE_ID,
      process.env.COLLECTION_ID,[Query.equal('username',username)]
    );
    console.log(process.env.DATABASE_ID);
    console.log(process.env.COLLECTION_ID);
    console.log("viewdata:", x);
  } catch (e) {
    console.log(e); // Corrected from 'error' to 'e'
  }
};
